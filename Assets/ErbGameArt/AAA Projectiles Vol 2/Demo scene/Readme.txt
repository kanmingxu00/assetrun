...
0) Download Post Effects throw Package manager end enable "Bloom". But "Bloom" from package manager makes all scene gray. 
   So you can use any other "Bloom". There are a couple of free ones in the Asset Store.

   *Deprecated asset*
1) Add "PostProcessingBehaviour.cs" on main Camera.
2) Set the "Post-effects" profile. ("\Assets\ErbGameArt\Demo scene\CC.asset")
3) You should turn on "HDR" on main camera for correct post-effects. (bloom post-effect works correctly only with HDR)
